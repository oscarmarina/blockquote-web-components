...
const mergeAlternatingValues = (
  firstArray: TemplateStringsArray,
  ...
  firstArray.map((strings, index) =>
    [
      ...
      isTemplateResult(secondArray[index])
        ? mergeAlternatingValues(
            (secondArray[index] as UncompiledTemplateResult).strings,
            (secondArray[index] as UncompiledTemplateResult).values
          ).join('')
        : secondArray[index],
    ].join('')
  );
...
class BlockquoteDirectiveSvgToDataImage extends Directive {
   ...
   */
  render(svgNodeInfo: unknown) {
    if (!svgNodeInfo || !isTemplateResult(svgNodeInfo)) {
      return noChange;
    }
    const svgTemplateStrings = (svgNodeInfo as UncompiledTemplateResult).strings;
