...
import { TemplateResultType, isTemplateResult } from 'lit/directive-helpers.js';
...
const isSvgType = svgTemplateResult => isTemplateResult(svgTemplateResult, TemplateResultType.SVG);

const mergeAlternatingValues = (firstArray = [], secondArray = []) =>
  firstArray.map((strings, index) =>
    [
      strings,
      isSvgType(secondArray[index])
...
class BlockquoteDirectiveSvgToDataImage extends Directive {
  render(svgNodeInfo) {
    if (!svgNodeInfo || !isSvgType(svgNodeInfo)) {
      return '';
    }
