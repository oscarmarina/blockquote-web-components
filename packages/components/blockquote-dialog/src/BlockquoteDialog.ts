...
export class BlockquoteDialog extends LitElement {
  treewalker: IterableIterator<HTMLElement> = walkComposedTree({
    root: this,
  ...
  dialogRef: Ref<HTMLDialogElement> = createRef<HTMLDialogElement>();
   ...
   * @override
   */
  static override styles = [styles, animations];
  ...
  _isConnectedCallbackResolve: () => void;
  _isConnectedCallback: Promise<void>;
  _firstFocusableChild?: HTMLElement | null;
  _lastFocusableChild?: HTMLElement | null;
  _nextClickIsFromContent: boolean;
  _overflowRoot: HTMLElement;
  _nativeDialogOpenObserver?: MutationController;
  _submitter?: HTMLElement;
  scroller?: HTMLElement | null;
  returnValue?: string | HTMLElement;
  type: string;
  label: string;
  labelledby: string;
  labelledbyVisible: boolean;
  ...
  getIsConnectedCallbackResolve() {
    return new Promise<void>((resolve) => {
      this._isConnectedCallbackResolve = resolve as () => void;
    });
  }
  ...
  override async connectedCallback() {
    ...
    this.scroller = this.shadowRoot?.querySelector<HTMLElement>('.scroller');
    const {value} = this.dialogRef;
    const [first, last] = getFirstAndLastFocusableChildren(this.treewalker);
  ...
  async show(): Promise<void> {
    ...
    const autofocusNode = this.querySelector<HTMLElement>('[autofocus]');
    if (autofocusNode) {
      autofocusNode?.focus();
  ...
  get _firstNodeFocusTrapTpl() {
    return html`
      <span
        ?hidden="${!this.dialogRef.value?.open}"
        tabindex="0"
  ...
  _handleSubmit(ev: SubmitEvent) {
    ...
    const isFormMethodDialog = (target as HTMLFormElement | null)?.method === 'dialog';
    const isSubmitterFormMethodDialog =
      (submitter as HTMLButtonElement | null)?.formMethod === 'dialog';

    if (!isFormMethodDialog && !isSubmitterFormMethodDialog) {
      return;
    }

    this._submitter = submitter ?? undefined;
    this.open = false;
  }

  _handleNativeOpen(records: MutationRecord[]) {
  ...
  _firstFocusTrap({relatedTarget}: FocusEvent) {
    (relatedTarget != null ? this._firstFocusableChild : this._lastFocusableChild)?.focus();
  }

  _lastFocusTrap({relatedTarget}: FocusEvent) {
