...
import type {BlockquoteDialog} from '../src/index.js';
...
describe('BlockquoteDialog', () => {
  let el: BlockquoteDialog;
  let elShadowRoot: string | undefined;
  ...
  describe('Event', () => {
    ...
    it('The "close" set returnValue', async () => {
      ...
      await el.updateComplete;
      await aTimeout(200);
      const returnValue = el.returnValue as HTMLElement;
  ...
  describe('Firefox/Safari fallback - is-closing exit animation', () => {
    beforeEach(async () => {
      vi.spyOn(CSS, 'supports').mockImplementation((...args: unknown[]) => {
        const [prop, val] = args;
  ...
  describe('focus', () => {
    ...
    it('focus on first focus trap sets focus to last element', async () => {
      ...
      await el.updateComplete;
      const firstFocusTrap = el.shadowRoot?.querySelector<HTMLElement>(
        'dialog > span:first-of-type'
      );
      firstFocusTrap?.focus();
    ...
    it('focus on first focus trap sets focus to first element when relatedTarget is undefined', async () => {
      ...
      firstFocusTrap?.dispatchEvent(new FocusEvent('focus', {relatedTarget: null}));
    ...
    it('focus on last focus trap sets focus to first element', async () => {
      ...
      await el.updateComplete;
      const firstButton = el.querySelector('#cancel');
      const lastFocusTrap = el.shadowRoot?.querySelector<HTMLElement>('dialog > span:last-of-type');
      lastFocusTrap?.focus();
      expect(document.activeElement).toBe(firstButton);
    });
    ...
    it('autofocus', async () => {
      const elAutofocus: BlockquoteDialog = await fixture(html`
