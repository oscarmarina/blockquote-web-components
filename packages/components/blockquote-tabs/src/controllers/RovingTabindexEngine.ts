interface FocusgroupEngineOptions {
...
export class RovingTabindexEngine {
  ...
  constructor({getItems, getOrientation}: FocusgroupEngineOptions) {
   ...
   */
  setEntryItem(entryItem: HTMLElement | undefined) {
    if (!entryItem) {
      return;
    }
   ...
   */
  #nextIndex(currentIndex: number, key: string): number | null {
  ...
  #onKeyDown = (ev: KeyboardEvent) => {
    ...
    const item = ev
      .composedPath()
      .find((node): node is HTMLElement => items.includes(node as HTMLElement));
    if (!item) {
      return;
    }
