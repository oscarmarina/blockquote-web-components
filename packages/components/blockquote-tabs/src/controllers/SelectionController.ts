...
interface SelectionControllerHost extends ReactiveControllerHost {
  selected: number;
}

interface SelectionItem extends HTMLElement {
  selected?: boolean;
}
 ...
 */
const clampFromOne = (value: number, length: number): number => {
...
export class SelectionController {
  ...
  #tabpanels: SelectionItem[] = [];

  #selectedIndex = 0;

  /** mismatches already surfaced (each one warns once) */
  #warnedMismatches = new Set<string>();

  host: SelectionControllerHost;

  constructor(host: SelectionControllerHost) {
    this.host = host;
   ...
   */
  setItems(tabs: HTMLElement[] = [], tabpanels: HTMLElement[] = []) {
    this.#tabs = [...tabs];
  ...
  #warnOnce(condition: boolean, message: string) {
