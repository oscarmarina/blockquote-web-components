...
export class BlockquoteTabs extends BlockquoteMixinSlotContent(LitElement) {
  ...
  _focusGroup: FocusGroupController;
  ...
  constructor() {
    ...
    this._scrollContentRef = createRef<HTMLDivElement>();
    this._tablistRef = createRef<HTMLDivElement>();
  ...
  get _selectedTab(): HTMLElement | undefined {
    return this._selection.selectedTab;
  }

  override firstUpdated(props: Map<PropertyKey, unknown>) {
    super.firstUpdated(props);
    const tabSlot = this.shadowRoot?.querySelector('[name="tab"]');
    const tabpanelSlot = this.shadowRoot?.querySelector('[name="tabpanel"]');

    this._tabList = (tabSlot as HTMLSlotElement | undefined)?.assignedElements() as HTMLElement[];
    this._tabpanelList = (
      tabpanelSlot as HTMLSlotElement | undefined
  ...
  _onSlotChanges = (ev: CustomEvent<SlotChangesDetail>) => {
    ev.stopPropagation();
    ev.preventDefault();
