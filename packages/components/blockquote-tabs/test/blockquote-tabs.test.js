...
describe('BlockquoteTabs', () => {
  ...
  describe('Default', () => {
    ...
    describe('Characterization (Phase 0)', () => {
      it('selectedchange bubbles, is not composed, and carries selection detail', async () => {
        ...
        const events = [];
        el.addEventListener('selectedchange', (ev) => events.push(ev));
        ...
        expect(ev.detail.tab).toBe(el.querySelectorAll('blockquote-tab')[1]);
        expect(ev.detail.tabpanel).toBe(el.querySelectorAll('blockquote-tabpanel')[1]);
      });

      it('deselects the previously selected tab and tabpanel on each commit', async () => {
        const tabs = [...el.querySelectorAll('blockquote-tab')];
        const tabpanels = [...el.querySelectorAll('blockquote-tabpanel')];
  ...
  describe('Selection (Phase 1)', () => {
    ...
    it('generates ids for tabs and tabpanels when missing', async () => {
      ...
      `);
      const [tab] = bare.querySelectorAll('blockquote-tab');
      const [tabpanel] = bare.querySelectorAll('blockquote-tabpanel');
    ...
    it('clamps the selection when the selected tab is removed', async () => {
      ...
      el.querySelectorAll('blockquote-tab')[2].remove();
      el.querySelectorAll('blockquote-tabpanel')[2].remove();
      ...
      expect(events[0].detail.tab).toBe(el.querySelectorAll('blockquote-tab')[1]);
    });
    ...
    it('warns when an author-supplied ARIA half does not close the pair', async () => {
      ...
      `);
      const [tab] = broken.querySelectorAll('blockquote-tab');
      const [tabpanel] = broken.querySelectorAll('blockquote-tabpanel');
  ...
  describe('FocusGroup native engine (Phase 3)', () => {
    ...
    it('overrides the axis to block for vertical tablists', async () => {
      ...
      `);
      const tablist = vertical.shadowRoot.querySelector('[role="tablist"]');
  ...
  describe('Disabled Tabs', () => {
    ...
    it('sets aria-disabled on disabled tab', () => {
      const tabs = [...disabledEl.querySelectorAll('blockquote-tab')];
