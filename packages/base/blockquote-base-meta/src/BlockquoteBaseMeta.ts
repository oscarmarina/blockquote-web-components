...
const blockquote: GlobalRecord =
  ((globalThisOrWindow as GlobalRecord)[BLOCKQUOTE] as GlobalRecord | undefined) ||
  Object.create(null);
const baseMeta: GlobalRecord =
  (blockquote[blockquoteBaseMeta] as GlobalRecord | undefined) || Object.create(null);
...
export class BlockquoteBaseMeta {
  ...
  private __key: unknown;

  constructor({type = 'default', key, value}: BlockquoteBaseMetaOptions = {}) {
    this.type = type;
    this.key = key;
    if (value != null) {
      this.value = value;
  ...
  static get types(): Map<unknown, Map<unknown, unknown>> {
    return baseMeta[types] as Map<unknown, Map<unknown, unknown>>;
  }

  static get uuid(): string {
    return baseMeta[uuid] as string;
  }
  ...
  set value(value: unknown) {
    if (this.type && this.key) {
      const typeMap = BlockquoteBaseMeta.types.get(this.type) ?? new Map<unknown, unknown>();
   ...
   */
  get mapList(): Map<unknown, unknown> {
   ...
   */
  get objectList(): Record<string, unknown> {
