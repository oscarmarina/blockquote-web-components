...
export class BlockquoteBaseEmbeddedWebviewSize extends LitElement {
  ...
  showOverflowSize: boolean;
  disabledSelectedSizeText: boolean;
  ...
  override willUpdate(changedProperties: PropertyValues<this>) {
    ...
    if (changedProperties.has('screenSizes')) {
  ...
  _setSelected(ev: Event) {
    ev.preventDefault();
    ev.stopPropagation();
    this.selected = Number((ev.target as HTMLElement).dataset.index);
