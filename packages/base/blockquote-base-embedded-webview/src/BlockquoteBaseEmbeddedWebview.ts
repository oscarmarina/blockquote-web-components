...
interface BlockquoteScreenSizeDetail {
  width: number;
  height: number;
  id: string;
  index: number;
...
const chevronDownIcon: TemplateResult = html`
  <svg
    aria-hidden="true"
    viewBox="0 0 24 24"
    stroke-width="2"
...
const openExternallyIcon: TemplateResult = html`
  <svg
    viewBox="0 0 20 20"
    fill-rule="evenodd"
    fill="currentcolor"
...
export class BlockquoteBaseEmbeddedWebview extends LitElement {
  ...
  selected: number;
  headingLevel: number;
  screenSizeSelected: number;
  limitHeight: boolean;
  embedded: Element | null | undefined;
  private _sources: SourceItem[];
  private _src: string | undefined;
  private _controlBottom: number | undefined;
  private __resetResizing = false;
  private __selectArrow = chevronDownIcon;
  private __readDataPos = {x: '0', y: '0', resizing: false, cursor: ''};
  private _embeddedResizeRef = createRef<HTMLElement>();
  ...
  override async connectedCallback() {
    ...
    this.addEventListener('webviewresize', (ev) => {
      const {detail} = ev as CustomEvent<WebviewResizeDetail>;
  ...
  _updateSize = ({detail}: CustomEvent<BlockquoteScreenSizeDetail>) => {
  ...
  _onChangeFile({target}: Event) {
    this.selected = (target as HTMLSelectElement).selectedIndex;
    this._src = this._sources[this.selected].src;
  }
}
